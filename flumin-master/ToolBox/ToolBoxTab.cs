...
namespace Silver.UI
{
    ...
    public class ToolBoxTab : ToolBoxItem
    {
        ...
        private Color                   _itemNormColor      = Color.Empty;
        private Color                   _itemSelColor       = Color.Empty;
        private Color                   _itemHoverColor     = Color.Empty;
        private Color                   _tabBackgroundColor = Color.Empty;
        ...
        */
        ...
        {
            get{return _itemNormColor;}
            set
            {
                if(value != _itemNormColor)
                {
                    _itemNormColor = value;
      ...
      [Category("ToolBoxTab")]
        ...
        {
            get{return _itemSelColor;}
            set
            {
                if(value != _itemSelColor)
                {
                    _itemSelColor = value;
                    if(null != _parent)
                    {
        ...
        protected int ItemSpacingEx
        {
            get
            {
                ...
                */
                if(_viewMode == ViewMode.LargeIcons)
                {
        ...
        private void On_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if(HitTest(e.X,e.Y))
            {
                ...
                else if(SystemColors.ControlText != _parent.TabHoverTextColor)
                {
            ...
            else
            {
                ...
                if(_mouseHover)
                {
                    ...
                    if(SystemColors.ControlText != _parent.TabHoverTextColor)
                    {
                        if(oldMouseHover != _mouseHover)
                        {
                            Invalidate();
        ...
        private void DoPainting(PaintEventArgs e)
        {
            ...
            {
                if (_tabBackgroundColor != Color.Empty) {
                    using (var brushBg = new SolidBrush(_tabBackgroundColor)) {
                        e.Graphics.FillRectangle(brushBg, _rectangle);
                    }
                }
                ...
                else
                {
                    ...
                    if(_mouseHover)
                    {
                        if(SystemColors.ControlText != _parent.TabHoverTextColor && null == txBrushes[1])
                        {
                            txBrushes[1] = new SolidBrush(_parent.TabHoverTextColor);
                        }
                        txBrush = null != txBrushes[1] ? txBrushes[1] : txBrush;
                    }
                    else if(_selected)
                    {
                        if(SystemColors.ControlText != _parent.TabSelectedTextColor && null == txBrushes[0])
                        {
                            txBrushes[0] = new SolidBrush(_parent.TabSelectedTextColor);
                        }
                        txBrush = null != txBrushes[0] ? txBrushes[0] : txBrush;
                    }
                    else
                    {
                        if(SystemColors.ControlText != _parent.TabNormalTextColor && null == txBrushes[2])
                        {
                            txBrushes[2] = new SolidBrush(_parent.TabNormalTextColor);
                        }
            ...
            if(ViewMode.List != this._viewMode)
            {
            ...
            if(!_itemArea.IsEmpty)
            {
                ...
                */
                ...
                try
                {
                    if(Color.Empty != _itemBgColor && _itemBgColor != _parent.BackColor)
                    {
                        txBrush = new SolidBrush(_itemBgColor);
            ...
            if(! (0 >= _itemArea.Height)  && null != _toolItems)
            {
                ...
                for(iLoop=_visibleTopIndex;iLoop<=_visibleBottomIndex;iLoop++)
                {
                    ...
                    if(item.MouseHover && !item.MouseDown)
                    {
                        if(null == bgBrushes[1])
                        {
                            bgBrushes[1] = new SolidBrush(_itemHoverColor.IsEmpty ? _parent.ItemHoverColor : _itemHoverColor);
                        }
                    ...
                    else if(item.Selected)
                    {
                        if(null == bgBrushes[2])
                        {
                            bgBrushes[2] = new SolidBrush(_itemSelColor.IsEmpty ? _parent.ItemSelectedColor : _itemSelColor);
                        }
                    ...
                    else
                    {
                        if(null == bgBrushes[0])
                        {
                            bgBrushes[0] = new SolidBrush(_itemNormColor.IsEmpty ? _parent.ItemNormalColor : _itemNormColor);
                        }
                    ...
                    {
                        ToolBox.DrawBorders(g,rect,true,_itemBorderColor.IsEmpty ? _parent.ItemBorderColor : _itemBorderColor);
                    }
                    else if(item.MouseHover)
                    {
                        ToolBox.DrawBorders(g,rect,false,_itemBorderColor.IsEmpty ? _parent.ItemBorderColor :  _itemBorderColor);
                    }
                    ...
                    if(_viewMode != ViewMode.List)
                    {
                       // rect.X = item.X;
                        ...
                        if(ViewMode.LargeIcons == _viewMode)
                        {
                            if(drewImage)
                            {
                        ...
                        else if(ViewMode.SmallIcons == _viewMode)
                        {
                            if(drewImage)
                            {
                                drawText = false;
                    ...
                    if(drawText)
                    {
                        ...
                        else
                        {
                            ...
                            if(item.MouseHover)
                            {
                                if(SystemColors.ControlText != _parent.ItemHoverTextColor && null == txBrushes[1])
                                {
                                    txBrushes[1] = new SolidBrush(_parent.ItemHoverTextColor);
                                }
                                txBrush = null != txBrushes[1] ? txBrushes[1] : txBrush;
                            }
                            else if(item.Selected)
                            {
                                if(SystemColors.ControlText != _parent.ItemSelectedTextColor && null == txBrushes[0])
                                {
                                    txBrushes[0] = new SolidBrush(_parent.ItemSelectedTextColor);
                                }
                                txBrush = null != txBrushes[0] ? txBrushes[0] : txBrush;
                            }
                            else
                            {
                                if(SystemColors.ControlText != _parent.ItemNormalTextColor && null == txBrushes[2])
                                {
                                    txBrushes[2] = new SolidBrush(_parent.ItemNormalTextColor);
                                }
                                txBrush = null != txBrushes[2] ? txBrushes[2] : txBrush;
                            }
                        }
                    ...
                    {
                        if(ViewMode.SmallIcons == _viewMode && !item.CaptionChecked)
                        {
                            item.CheckCaption(g,_parent.Font,strFormat,Rectangle.Empty);
                        }
                    }

                    if(_viewMode == ViewMode.List && rect.Bottom >= _itemArea.Bottom)
                    {
                        // Exit loop.
                        iLoop = _toolItems.Count + 1;
                    }
        ...
        private Point GetItemLocation(int index, int itemWidth, int itemHeight, int maxWidth)
        {
            ...
            if(null != _toolItems)
            {
                if(0 == index)
                {
                    itemLoc.X = ViewMode.List == _viewMode ? 0 : 4;
                    itemLoc.Y = ViewMode.List == _viewMode ? 1 : 2;
                }
                ...
                if(bOk && ViewMode.List != _viewMode && (_onlyOneItemPerRow || _parent.ShowOnlyOneItemPerRow))
                {
