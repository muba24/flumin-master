...
//
//          01/10/2005 - Splitted code to different files.
...
//                     - Two more view modes added for toolbox items.
//                     - (LargIcons, SmallIcons)
...
//
// Legal   : You may use/modify the source library FREELY for personal
...
namespace Silver.UI
{
    ...
    public class ToolBox : UserControl
    {
        ...
        private Color                _itemBgColor        = Color.Empty;
        private Color                _itemBorderColor    = Color.Empty;         // #0A246A XP
        private Color                _selectedItemColor  = Color.White;         // #D2D4DA XP
        private Color                _itemHoverColor     = SystemColors.Control;// #B6BDD2 XP
        private Color                _itemNormalColor    = SystemColors.Control;

        private Color                _tabTxtNormalColor  = Color.Empty;
        private Color                _tabTxtHoverColor   = Color.Empty;
        private Color                _tabTxtSelectColor  = Color.Empty;

        private Color                _itemTxtNormalColor = Color.Empty;
        private Color                _itemTxtHoverColor  = Color.Empty;
        private Color                _itemTxtSelectColor = Color.Empty;
        private Color                _defTxtBoxBgColor   = Color.Empty;
        private Color                _defTxtBoxTxColor   = Color.Empty;
        ...
        private void Initialize()
        {
            ...
            _defTxtBoxBgColor       = _textBox.BackColor;
            _defTxtBoxTxColor       = _textBox.ForeColor;
            _textBox.Font           = Font;
        ...
        {
            get{return _tabTxtSelectColor.IsEmpty ? SystemColors.ControlText : _tabTxtSelectColor;}
            set
            {
                if(value != _tabTxtSelectColor)
                {
                    if(value != SystemColors.ControlText)
                    {
                        _tabTxtSelectColor = value;
                    }
                    else
                    {
                        _tabTxtSelectColor = Color.Empty;
                    }
                    Invalidate();
                }
            }
        ...
        {
            get{return _tabTxtNormalColor.IsEmpty ? SystemColors.ControlText : _tabTxtNormalColor;}
            set
            {
                if(value != _tabTxtNormalColor)
                {
                    if(value != SystemColors.ControlText)
                    {
                        _tabTxtNormalColor = value;
                    }
                    else
                    {
                        _tabTxtNormalColor = Color.Empty;
                    }
                    Invalidate();
                }
            }
        ...
        {
            get{return _tabTxtHoverColor.IsEmpty ? SystemColors.ControlText : _tabTxtHoverColor;}
            set
            {
                if(value != _tabTxtHoverColor)
                {
                    if(value != SystemColors.ControlText)
                    {
                        _tabTxtHoverColor = value;
                    }
                    else
                    {
                        _tabTxtHoverColor = Color.Empty;
                    }
                    Invalidate();
                }
            }
        ...
        {
            get{return _itemTxtSelectColor.IsEmpty ? SystemColors.ControlText : _itemTxtSelectColor;}
            set
            {
                if(value != _itemTxtSelectColor)
                {
                    if(value != SystemColors.ControlText)
                    {
                        _itemTxtSelectColor = value;
                    }
                    else
                    {
                        _itemTxtSelectColor = Color.Empty;
                    }
                    Invalidate();
                }
            }
        ...
        {
            get{return _itemTxtNormalColor.IsEmpty ? SystemColors.ControlText : _itemTxtNormalColor;}
            set
            {
                if(value != _itemTxtNormalColor)
                {
                    if(value != SystemColors.ControlText)
                    {
                        _itemTxtNormalColor = value;
                    }
                    else
                    {
                        _itemTxtNormalColor = Color.Empty;
                    }
                    Invalidate();
                }
            }
        ...
        */
        ...
        internal void RenameItem(ToolBoxItem item)
        {
            ...
            try
            {
                if(item.GetType() != typeof(ToolBoxTab))
                {
                    ...
                    {
                        _selectedTab.View = ViewMode.List;
                ...
                if(bOK && item.Renamable)
                {
                    ...
                    if(_useItemClrInRename)
                    {
                        if(!bIsTab)
                        {
                            if(item.Selected)
                            {
                                _textBox.BackColor = _selectedItemColor.IsEmpty  ? _textBox.BackColor : _selectedItemColor;
                                _textBox.ForeColor = _itemTxtSelectColor.IsEmpty ? _textBox.ForeColor : _itemTxtSelectColor;
                            }
                            else
                            {
                                _textBox.ForeColor = _itemTxtNormalColor.IsEmpty ? _textBox.ForeColor : _itemTxtNormalColor;
                                _textBox.BackColor = _itemNormalColor.IsEmpty    ? _textBox.BackColor : _itemNormalColor;
                            }
                        }
                        else
                        {
                            if(item.Selected)
                            {
                                _textBox.ForeColor = _tabTxtSelectColor.IsEmpty ? _textBox.ForeColor : _tabTxtSelectColor;
                            }
                            else
                            {
                                _textBox.ForeColor = _tabTxtNormalColor.IsEmpty ? _textBox.ForeColor : _tabTxtNormalColor;
                            }
        ...
        public DragDropEffects DoDragDropItem(ToolBoxItem item, DragDropEffects allowedEffects)
        {
            ...
            try
            {
                ...
                {
                    OnBeginDragDrop(item,new PreDragDropEventArgs(dobj,item.Object));
                }
            }
            catch
            {
        ...
        private void RestoreTextBoxColors()
        {
            if(_useItemClrInRename)
            {
                _textBox.BackColor = _defTxtBoxBgColor;
                _textBox.ForeColor = _defTxtBoxTxColor;
            }
        }
        ...
        {
            RenameFinishedEventArgs e   = null;
            ...
            try
            {
                ...
                {
                    e   = new RenameFinishedEventArgs(_textBox.Text,byEscapeKey);
        ...
        private void HandleScrollTimerElapsed(ScrollDirection dir, ToolBoxTab tab, ToolScrollButton sb)
        {
            if(null != tab)
            {
                tab.ScrollItems(dir);
        ...
        private void HandleToolBoxTabDrop(DragEventArgs e)
        {
            ...
            try
            {
                dragTab = (ToolBoxTab)e.Data.GetData(typeof(Silver.UI.ToolBoxTab));
        ...
        private void HandleToolBoxItemDrop(DragEventArgs e)
        {
            ...
            try
            {
                dragItem = (ToolBoxItem)e.Data.GetData(typeof(Silver.UI.ToolBoxItem));
        ...
        protected override void OnSizeChanged(EventArgs e)
        {
            ...
            foreach(ToolBoxTab tab in _toolBoxTabs)
            {
                tab.UpdateItemRects(ViewMode.List != tab.View,true,false);
            }
        ...
        protected override void OnDragOver(DragEventArgs e)
        {
            if(_allowDragSwap)
            {
                if(e.Data.GetDataPresent(typeof(Silver.UI.ToolBoxTab)))
                {
                    HandleToolBoxTabDragOver(e);
                }
                else if(e.Data.GetDataPresent(typeof(Silver.UI.ToolBoxItem)))
                {
        ...
        {
            HandleScrollTimerElapsed(ScrollDirection.Up,_selectedTab,_upScroll);
        }

        private void OnTimer_DnScrollElapsed(object sender, EventArgs e)
        {
            HandleScrollTimerElapsed(ScrollDirection.Down,_selectedTab,_dnScroll);
        }
        ...
        public bool XmlSerialize(XmlNode xmlNode)
        {
            ...
            try
            {
                ...
                if(!_itemBgColor.IsEmpty)
                {
                    element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_itemBgColor)));
                }
                element0.AppendChild(element1);

                // Item border color
                element1 = xmlDoc.CreateElement(Xml_ItemBorderColor_Node);
                if(!_itemBorderColor.IsEmpty)
                {
                    element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_itemBorderColor)));
                }
                element0.AppendChild(element1);

                // Normal item color
                element1 = xmlDoc.CreateElement(Xml_NormItemColor_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_itemNormalColor)));
                element0.AppendChild(element1);

                // Selected item color
                element1 = xmlDoc.CreateElement(Xml_SelItemColor_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_selectedItemColor)));
                element0.AppendChild(element1);

                // Hover item color
                element1 = xmlDoc.CreateElement(Xml_HoverItemColor_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_itemHoverColor)));
                element0.AppendChild(element1);

                // Normal item text color
                element1 = xmlDoc.CreateElement(Xml_NormItemTxtColor_Node);
                if(!_itemTxtNormalColor.IsEmpty)
                {
                    element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_itemTxtNormalColor)));
                }
                element0.AppendChild(element1);

                // Selected item text color
                element1 = xmlDoc.CreateElement(Xml_SelItemTxtColor_Node);
                if(!_itemTxtSelectColor.IsEmpty)
                {
                    element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_itemTxtSelectColor)));
                }
                element0.AppendChild(element1);
                ...
                if(!_tabTxtNormalColor.IsEmpty)
                {
                    element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_tabTxtNormalColor)));
                }
                element0.AppendChild(element1);

                // Selected tab text color
                element1 = xmlDoc.CreateElement(Xml_SelTabTxtColor_Node);
                if(!_tabTxtSelectColor.IsEmpty)
                {
                    element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_tabTxtSelectColor)));
                }
                element0.AppendChild(element1);

                // Hover tab text color
                element1 = xmlDoc.CreateElement(Xml_HoverTabTxtColor_Node);
                if(!_tabTxtHoverColor.IsEmpty)
                {
                    element1.AppendChild(xmlDoc.CreateTextNode(ColorToHtmlString(_tabTxtHoverColor)));
                }
                element0.AppendChild(element1);
                ...
                for(iLoop1=0;iLoop1<_toolBoxTabs.Count;iLoop1++)
                {
                    ...
                    // Item bg color
                    if(!tab.ItemBackgroundColor.IsEmpty)
                    {
                        tabElement.SetAttribute(Xml_ItemBgClr_Attr,ColorToHtmlString(tab.ItemBackgroundColor));
                    }

                    //Item border color
                    if(!tab.ItemBorderColor.IsEmpty)
                    {
                        tabElement.SetAttribute(Xml_ItemBorderClr_Attr,ColorToHtmlString(tab.ItemBorderColor));
                    }

                    //Item normal color
                    if(!tab.ItemNormalColor.IsEmpty)
                    {
                        tabElement.SetAttribute(Xml_ItemNormClr_Attr,ColorToHtmlString(tab.ItemNormalColor));
                    }

                    //Item selected color
                    if(!tab.ItemSelectedColor.IsEmpty)
                    {
                        tabElement.SetAttribute(Xml_ItemSelClr_Attr,ColorToHtmlString(tab.ItemSelectedColor));
                    }

                    //Item hover color
                    if(!tab.ItemHoverColor.IsEmpty)
                    {
                        tabElement.SetAttribute(Xml_ItemHoverClr_Attr,ColorToHtmlString(tab.ItemHoverColor));
                    }
                    ...
                    try
                    {
                        ...
                        {
                            OnSerializeObject(tab,new XmlSerializationEventArgs(tab.Object,tabElement,false));
                        }
                    }
                    catch
                    {
                    ...
                    for(iLoop2=0;iLoop2<tab.ItemCount;iLoop2++)
                    {
                        ...
                        try
                        {
                            ...
                            if(null != OnSerializeObject)
                            {
                                OnSerializeObject(item,new XmlSerializationEventArgs(item.Object,itemElement,false));
                            }
                        }
                        catch
                        {
        ...
        public bool XmlDeSerialize(XmlElement element)
        {
            ...
            XmlSerializationEventArgs evtArgs      = null;
            ...
            try
            {
                ...
                _itemBgColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Item border color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_ItemBorderColor_Node);
                _itemBorderColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Normal item color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_NormItemColor_Node);
                _itemNormalColor = ColorFromHtmlString(element1.InnerText,SystemColors.Control);

                // Selected item color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_SelItemColor_Node);
                _selectedItemColor = ColorFromHtmlString(element1.InnerText,Color.White);

                // Hover item color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_HoverItemColor_Node);
                _itemHoverColor = ColorFromHtmlString(element1.InnerText,SystemColors.Control);

                // Normal item text color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_NormItemTxtColor_Node);
                _itemTxtNormalColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Selected item text color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_SelItemTxtColor_Node);
                _itemTxtSelectColor = ColorFromHtmlString(element1.InnerText,Color.Empty);
                ...
                _tabTxtNormalColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Selected tab text color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_SelTabTxtColor_Node);
                _tabTxtSelectColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Hover tab text color
                element1     = (XmlElement)element0.SelectSingleNode(Xml_HoverTabTxtColor_Node);
                _tabTxtHoverColor = ColorFromHtmlString(element1.InnerText,Color.Empty);
                ...
                //Up Scroll
                itemElement       = (XmlElement)element1.SelectSingleNode(ScrollDirection.Up.ToString());
                ...
                // Down Scroll
                itemElement       = (XmlElement)element1.SelectSingleNode(ScrollDirection.Down.ToString());
                ...
                for(iLoop1=0;iLoop1<tabNodes.Count;iLoop1++)
                {
                    ...
                    try
                    {
                        tab.View = (ViewMode)Enum.Parse(typeof(Silver.UI.ViewMode),tabElement.GetAttribute(Xml_ViewMode_Attr),true);
                    }
                    catch
                    {
                        tab.View = ViewMode.List;
                    }
                    ...
                    // Item bg color
                    tab.ItemBackgroundColor   = ColorFromHtmlString(tabElement.GetAttribute(Xml_ItemBgClr_Attr),Color.Empty);

                    // Item border color
                    tab.ItemBorderColor       = ColorFromHtmlString(tabElement.GetAttribute(Xml_ItemBorderClr_Attr),Color.Empty);

                    //Item normal color
                    tab.ItemNormalColor       = ColorFromHtmlString(tabElement.GetAttribute(Xml_ItemNormClr_Attr),Color.Empty);

                    //Item selected color
                    tab.ItemSelectedColor     = ColorFromHtmlString(tabElement.GetAttribute(Xml_ItemSelClr_Attr),Color.Empty);

                    //Item hover color
                    tab.ItemHoverColor        = ColorFromHtmlString(tabElement.GetAttribute(Xml_ItemHoverClr_Attr),Color.Empty);
                    ...
                    {
                        evtArgs = new XmlSerializationEventArgs(null,tabElement,true);
                    ...
                    for(iLoop2=0;iLoop2<itemNodes.Count;iLoop2++)
                    {
                        ...
                        try
                        {
                            ...
                            if(null != OnDeSerializeObject)
                            {
                                evtArgs = new XmlSerializationEventArgs(null,itemElement,true);
