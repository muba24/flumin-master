...
namespace NodeSystemLib {
    ...
    public class RingBuffer<T> : IEnumerable<T>, ICloneable {
    ...
    public sealed class SignalRingBuffer {
        ...
        public bool IgnoreOverflow { get; set; } = false;
        ...
        /// <param name="size">The number of elements to enqueue</param>
        private int Enqueue(double[] buffer, int offset, int size) {
            if (size == 0)
                return 0;
            ...
            lock (this) {
                if ((Length + size) > _fInternalBuffer.Length) {
                    if (!SizeFixed) {
                        ...
                        if (!IgnoreOverflow) {
                            NodeSystemSettings.Instance.SystemHost.ReportError("Node: Buffer size too small. Lost data");
                        }
        ...
        private int Peek(double[] buffer, int offset, int size) {
            lock (this) {
                if (size > Length)
                    size = Length;
        ...
        public int Peek(TimeLocatedBuffer buffer, int length = -1) {
            lock (this) {
                ...
                else if (length > buffer.Length) length = buffer.Length;
        ...
        public void LoadState(object state) {
            lock (this) {
                if (!(state is SignalRingBuffer)) {
                    throw new InvalidCastException($"State object is not of type {nameof(SignalRingBuffer)}");
                }
                ...
                IgnoreOverflow = buffer.IgnoreOverflow;
                SizeFixed      = buffer.SizeFixed;
